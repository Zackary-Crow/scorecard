...
#finds n many numbers inside of box
    ...
    if( cv2.boundingRect(group[0])[0] < (600-xval) +100):
        return            
    val = len(group)
...
for i,j in enumerate(section[:-1]):
    groups, sectionImg = computeSection(section[i],section[i+1])
    for group in groups:
        ...
        for digit in arr:
            ...
            item.append(val)
        if len(item) == 2:
            predictions.append(item[0]+(item[1]/10))
        else:
            predictions.append(sum(d * 10**i for i, d in enumerate(item[::-1])))
        print(predictions[-1])
